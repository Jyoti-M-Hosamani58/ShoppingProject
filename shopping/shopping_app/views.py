...
from shopping_app.models import Login,Items, Sales, Return,UserReg
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                sdiscountAmt = request.POST.get('sdiscountAmt')
...
def printInvoice(request, invoice_no):
    ...
    # Retrieve the discount amounts (discountAmt and sdiscountAmt)
    discount_amount = items.first().discountAmt if hasattr(items.first(), 'discountAmt') else 0.0
    sdiscount_amount = items.first().sdiscountAmt if hasattr(items.first(), 'sdiscountAmt') else 0.0

    # Calculate the discounted total using both discount amounts
    discounted_total = grand_total - discount_amount - sdiscount_amount
...
def viewSales(request):
    ...
    sales_summary = {}
    grand_total = 0.0  # Initialize grand_total to accumulate total_amt across invoices
    ...
    for sale in sales:
        ...
        discount = float(sale.discount) if hasattr(sale, 'discount') else 0.0
        discount_amt = float(sale.discountAmt) if hasattr(sale, 'discountAmt') else 0.0
        sdiscount_amt = float(sale.sdiscountAmt) if hasattr(sale, 'sdiscountAmt') else 0.0
        ...
        sales_summary[invoice_no]['total_discount'] += discount
        sales_summary[invoice_no]['discountAmt'] += discount_amt
        sales_summary[invoice_no]['sdiscountAmt'] += sdiscount_amt

        # Calculate total amount including all discounts
        total_discount = discount_amt + sdiscount_amt
        total_amt = item_total - total_discount
        sales_summary[invoice_no]['total_amt'] += total_amt

    # Calculate grand_total across all invoices
    for summary in sales_summary.values():
        grand_total += summary['total_amt']

    # Pass sales summary and grand_total to the template
    return render(request, 'viewSales.html', {'sales_summary': sales_summary, 'grand_total': grand_total})
...
def register(request):
    if request.method=="POST":
        ...
        if ucount>=1:
            ...
            Login.objects.create(utype=utype, username=email, password=password)
...
def staffsale(request):
    return render(request,'staffSale.html')

@csrf_exempt
def staffsave_sales(request):
    if request.method == 'POST':
        try:
            with transaction.atomic():
                now = datetime.now()
...
def stffprintInvoice(request, invoice_no):
    # Get all sales records related to this invoice number
    items = Sales.objects.filter(invoiceNo=invoice_no)
    ...
    # Render the invoice template with all calculated values
    return render(request, 'stffprintInvoice.html', {
        'invoice_details': items.first(),  # Use the first record for invoice details
        'items': items,
        'total_qty': total_qty,
        'subtotal': subtotal,
...
def staffReturn(request):
    return render(request,'staffReturn.html')

@csrf_exempt
def staffsave_return(request):
    if request.method == 'POST':
        try:
            with transaction.atomic():
                now = datetime.now()
